...
export default function RemindersScreen() {
  ...
  const showBatteryOptimizationPrompt = async () => {
    ...
    try {
      const seen = await AsyncStorage.getItem('hasSeenBatteryPrompt');
      if (seen === 'true') return;
      Alert.alert(
      ...
      <View style={styles.catRow}>
        {CATEGORIES.map((cat) => (
          <TouchableOpacity
            ...
            style={[
              styles.catBtn,
              selectedCategory === cat.label && { backgroundColor: cat.bg, borderColor: cat.color },
            ]}
            onPress={() => setSelectedCategory(cat.label)}
            activeOpacity={0.75}
          >
            <Text style={[styles.catBtnText, selectedCategory === cat.label && { color: cat.color }]}>
              {cat.emoji} {cat.label}
            </Text>
          </TouchableOpacity>
        ))}
        ...
        <View style={styles.weekRow}>
          {weekdays.map((day, i) => (
            <TouchableOpacity
              ...
              activeOpacity={0.75}
            >
              <Text style={[styles.weekBtnText, selectedDays.includes(i) && { color: '#f472b6' }]}>
                {day}
              </Text>
            </TouchableOpacity>
          ))}
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontSize: 34, fontWeight: '800', color: '#f1f5f9', letterSpacing: -1 },
  ...
  catBtnText: { fontSize: 12, fontWeight: '700', color: '#94a3b8' },

  weekRow: { flexDirection: 'row', justifyContent: 'center', flexWrap: 'wrap', gap: 6, marginBottom: 12 },
  weekBtn: {
    paddingVertical: 7,
  ...
  weekBtnText: { fontSize: 12, fontWeight: '700', color: '#94a3b8' },
  ...
  timeBtnText: { flex: 1, fontSize: 14, fontWeight: '500', color: '#94a3b8' },

  dividerRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 12, gap: 10 },
  divider:     { flex: 1, height: 1, backgroundColor: '#1e1e30' },
  dividerText: { fontSize: 11, color: '#94a3b8', fontWeight: '700', letterSpacing: 0.8, textTransform: 'uppercase' },
  ...
  taskAccentBar: { width: 4, alignSelf: 'stretch', borderTopRightRadius: 2, borderBottomRightRadius: 2 },
  taskName:      { fontSize: 15, fontWeight: '700', color: '#f1f5f9', marginBottom: 8 },
  tagRow:        { flexDirection: 'row', alignItems: 'center', gap: 8, flexWrap: 'wrap' },
  catPill:       { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8, borderWidth: 1 },
  catPillText:   { fontSize: 12, fontWeight: '700' },
  timePill:      { backgroundColor: '#1a1a2e', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8 },
  timePillText:  { fontSize: 12, color: '#cbd5e1', fontWeight: '500' },
  daysRow:       { flexDirection: 'row', marginTop: 8, gap: 5, flexWrap: 'wrap' },
  miniDay:       { paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6, borderWidth: 1 },
  ...
  emptyText:  { fontSize: 15, color: '#94a3b8', textAlign: 'center', lineHeight: 24, fontWeight: '500' },
});